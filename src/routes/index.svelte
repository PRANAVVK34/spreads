
<script>



    let nav = [

    {nav:'My Spreads',image:'https://lh3.googleusercontent.com/EbXw8rOdYxOGdXEFjgNP8lh-YAuUxwhOAe2jhrz3sgqvPeMac6a6tHvT35V6YMbyNvkZL4R_a2hcYBrtfUhLvhf-N2X3OB9cvH4uMw=w1064-v0'},
    {nav:'Contributors',image:"https://upload.wikimedia.org/wikipedia/en/thumb/3/35/Information_icon.svg/768px-Information_icon.svg.png"},
    {nav:'Payout',image:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT769bgfTT-i73ZB4S0VR6Z_A4YEZyAOa92YQ&usqp=CAU"},
    {nav:'Logout',image:"https://upload.wikimedia.org/wikipedia/en/thumb/3/35/Information_icon.svg/768px-Information_icon.svg.png"}
    ]



    let tableHeadings=[
        'spreads',
        'contributors',
        'Revenue',
        'Status',
        ""
    ]

    let tableData = [
        {dt:'02',image:'.'},
        {dt:'$200',image:'.'},
        {dt:'Live',image:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT769bgfTT-i73ZB4S0VR6Z_A4YEZyAOa92YQ&usqp=CAU'},
        {dt:'Preview',image:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT769bgfTT-i73ZB4S0VR6Z_A4YEZyAOa92YQ&usqp=CAU'},
    ]

    let show = false
    let navShow = false
    let inWidth 

    if (typeof window !== 'undefined') {
    
   inWidth = window.innerWidth
   console.log(inWidth);
    
}
   


    let toggleHandler = () =>{
        show = true
    }

    let closeHandler = () =>{
        show = false
    }

    let navShower = () =>{
        navShow = !navShow
    }


</script>


<section class="flex h-screen overflow-hidden">
    <div class="flex flex-col absolute left-4 top-8 gap-1 cursor-pointer md:hidden z-10" on:click={navShower}>
        <div class="w-6 h-1 bg-white-400"></div>
        <div class="w-6 h-1 bg-white-400"></div>
        <div class="w-6 h-1 bg-white-400"></div>
    </div>
    <!-- navigation section -->
   {#if navShow || inWidth > 640}

   <section class=" md:w-1/4 bg-nav pl-20 xl:pl-20 md:pl-6 xl:text-base lg:text-sm md:text-xs  md:relative absolute h-screen w-screen">

    <div class="flex gap-4  pt-6 pl-4">
        <img class="w-16 h-16 rounded" src="data:image/jpeg;base64," alt="">
        <div class="flex flex-col ">
            <h2 class="text-white-400">@AverageAndrea</h2>
            <p class="text-white-100">Joined 2 days ago</p>
        </div>

    </div>

    <hr class=" border-white-100 w-full mt-4">

    <nav>
        <ul class="text-white-400 mt-4">
            {#each nav as n}
          
            <li class="mt-4 flex text-left pl-4">
                <img class="w-4 mr-6" src={n.image} alt="" srcset="">
                <a href="/">{n.nav}</a></li>
            <hr class=" border-white-100 w-full mt-4">
            {/each}
           
        </ul>
    </nav>

    <div class="flex gap-2 absolute bottom-14">
        <img class="w-12 h-12" src="./favicon.png" alt="">
        <h1 class="text-white-400 text-xl">spread</h1>
    </div>

</section>
       
   {/if}

    <!-- main section -->
    <section class="w-screen md:w-3/4 bg-main">

        <div class="flex justify-between px-12 md:pt-11 pt-8">
            <h1 class="font-medium xl:text-2xl md:text-lg">My Spreads</h1>
            <button class="border border-nav bg-blue px-2 md:px-4 py-2 text-white-400 rounded xl:text-base md:text-xs text-xs">New Spread</button>
        </div>

        <hr class=" border-white-100 w-full mt-4">


        <table class="w-6/12 xl:w-10/12 lg:w-6/12 md:w-4/12 md:ml-6 ml-1">

     
            <tr class=" text-xs font-light xl:text-sm lg:text-sm md:text-xs ">
                {#each tableHeadings as tbh}
                <th class="xl:px-12 lg:px-7 md:px-5 px-2 py-10 text-left">{tbh}</th>
                {/each}
            </tr>
       
                 

            <tr class="text-xs bg-white-600 border cursor-pointer rounded-lg xl:text-base lg:text-sm md:text-xs" on:click={toggleHandler}>

                <td class="xl:px-12 lg:px-7 md: px-2 py-4 ">Spread #1 <br>
                <span class=" text-white-100 text-sm md:block hidden">http//spread.stream/embeded
                </span>
               
                </td>

              

                {#each tableData as tbd}
           
               
                    
                    <td class="xl:px-12 lg:px-7 md:px-2 px-5 pt-1" >
                    
                    {tbd.dt}
                    </td> 
              
              
                {/each}

            </tr>

        </table>



           {#if show}
               <div class="w-screen h-screen absolute left-0 top-0 bg-white-100 bg-opacity-40">
                   
                
                    <div class="relative left-1/3 top-1/3 w-1/3 border-4 p-8 border-main bg-nav text-white-400 shadow-2xl z-10">
                        <p on:click={closeHandler} class="border w-7 absolute -right-4 -top-4 bg-nav text-white-400 pl-2 cursor-pointer">X</p>
                        <div class="flex gap-2">
                            <h2>FIFA22 12 MAY FED</h2>
                            <img class="w-2" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT769bgfTT-i73ZB4S0VR6Z_A4YEZyAOa92YQ&usqp=CAU" alt="">
                        </div>
                        <hr class="my-4 opacity-40 ">
                        <div>
                            <p>Add this to obs</p>
                            <p class="border rounded pl-3 pr-2 w-52 my-4 bg-white-100">https//embeded/obs/3244</p>
                            <div class="flex gap-2">
                                <img class="w-2" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT769bgfTT-i73ZB4S0VR6Z_A4YEZyAOa92YQ&usqp=CAU" alt="">
                                <p>see how it works</p>
                            </div>
                        </div>
                        <br>
                        <div>
                            <p>share this to your followers</p>
                            <p class="border rounded px-4 w-64 bg-white-100">https//abcd/obs/tanmay/64537</p>
                        </div>
                    </div>

                    <div class="absolute left-1/3 ml-2 top-1/3 w-1/3 h-80 border-4 p-8 border-main bg-nav z-0">

                    </div>
              

               </div>
           {/if}

            
        


    </section>
</section>
