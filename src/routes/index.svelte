<script>

    let nav = [
        'My Spreads',
        'Contributors',
        'Payout',
        'Logout'
    ]


    let tableHeadings=[
        'spreads',
        'contributors',
        'Revenue',
        'Status',
        ""
    ]

    let tableData = [
        'Spread #1',
        '02',
        '$200',
        'Live',
        'Preview'
    ]

</script>

<section class="flex h-full">
    <!-- navigation section -->
    <section class="w-1/4 bg-nav pl-20 ">

        <div class="flex gap-4  pt-6 pl-4">
            <img class="w-16 h-16 rounded" src="data:image/jpeg;base64," alt="">
            <div class="flex flex-col ">
                <h2 class="text-white-400">@AverageAndrea</h2>
                <p class="text-white-100">Joined 2 days ago</p>
            </div>

        </div>

        <hr class=" border-white-100 w-full mt-4">

        <nav>
            <ul class="text-white-400 mt-4">
                {#each nav as n}
                <li class="mt-4 flex text-left pl-4"><a href="/">{n}</a></li>
                <hr class=" border-white-100 w-full mt-4">
                {/each}
               
            </ul>
        </nav>

        <div class="flex gap-2 absolute bottom-14">
            <img class="w-12 h-12" src="./favicon.png" alt="">
            <h1 class="text-white-400 text-xl">spread</h1>
        </div>

    </section>

    <!-- main section -->
    <section class="w-3/4 bg-main ">

        <div class="flex justify-between px-12 pt-11">
            <h1 class="font-medium text-2xl">My Spreads</h1>
            <button class="border border-nav bg-blue px-4 py-2 text-white-400 rounded">New Spread</button>
        </div>

        <hr class=" border-white-100 w-full mt-4">


        <div>
            
              <table>
                <tr class=" text-sm font-light">
                    {#each tableHeadings as tbh}
                    <th class="px-14 py-10 text-left">{tbh}</th>
                    {/each}
                </tr>
           
            
           
                <tr class="text-lg ">
                    {#each tableData as tbd}
                    <td class="px-14 ">{tbd}</td>
                    {/each}
                
                </tr>

                <tr class="">
                    <td class="px-14 text-white-100 text-sm">http//spread.stream/embeded</td>
                    
                </tr>
             
              
            
              </table>
            
        </div>


       
       

    </section>
</section>
